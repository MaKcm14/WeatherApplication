...
bool NRequest::TCacheManager::IsDataExpired(const std::string& city) {
    ...
    std::vector<std::vector<std::string>> unexpiredData;

    try {
        unexpiredData = DataBase->ISelect(std::string("SELECT * FROM weather\r\nWHERE city = '") + city +
            std::string("' AND expires >= '") + GetCurTimeWithShift(0) + "'\r\n");
...
std::string NRequest::TCacheManager::GetData(const std::string& city) {
    ...
    try {
        data = DataBase->ISelect(std::string("SELECT * FROM weather")+
            "\r\nWHERE city='" + city + "'\r\n");
...
bool NRequest::TCacheManager::CheckExistingData(const std::string& city) {
    ...
    try {
        ...
        throw TRequestException("the query of the checking the city's existing "
    ...
    if (CheckExistingData(city)) {
        try {
            DataBase->IUpdate(std::string("UPDATE weather\r\n") +
                "SET description='" + weathDesc + "', expires='" + GetCurTimeWithShift(15) + "'\r\n" +
                "WHERE city='" + city + "'\r\n");

        } catch (NDataBase::TDataBaseException& excp) {
            logger << TLevel::Error << "~ InsertOrUpdate() error: the query of the updating";
        ...
        try {
            DataBase->IInsert(std::string("INSERT INTO weather ") + 
                ...
                GetCurTimeWithShift(15) +"')\r\n");
        
        } catch (NDataBase::TDataBaseException& excp) {
            logger << TLevel::Error << "~ InsertOrUpdate() error: the query of the inserting ";
            logger << "the city's weather description wasn't processed correctly: ";
            logger << excp.what() << "\n\n";
